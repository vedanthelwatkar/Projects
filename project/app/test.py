...
# Create the index
index = faiss.IndexFlatL2(embeds.shape[1])  # L2 distance

# Add all the vectors to the index
index.add(embeds)

# Query function using faiss
def search_andrews_article(query):
    # Get the query's embedding
    query_embed = co.embed(texts=[query]).embeddings
    query_embed = np.array(query_embed)  # Convert to NumPy array

    # Search the index for nearest neighbors
    _, similar_item_ids = index.search(query_embed, 10)
