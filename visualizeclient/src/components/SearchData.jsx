...
export const SearchData = ({ updateTransactions,selectedMonth }) => {

  const [q,setQ] = useState("")

  const handleSearch = async () => {
    let url = `http://localhost:8000/search?q=${q}`
    let response = await axios.post(url,{month:selectedMonth})
  ...
  <div class="search-box">
    <div class="search-field">
      <input placeholder="Search..." class="input" type="text" value = {q} onChange={(e)=>{setQ(e.target.value)}}/>
      <div class="search-box-icon">
        <button class="btn-icon-content" onClick={handleSearch}>
          <i class="search-icon">
